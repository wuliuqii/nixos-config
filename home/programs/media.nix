...
in
{
  ...
  home.file = {
    ".config/mpd/mpd.conf".text = ''
      ...
      log_file           "${dataDir}/mpd.log"
      pid_file           "${dataDir}/mpd.pid"
    ...
    ".config/go-musicfox/go-musicfox.ini".text = ''
      [startup]
      # 是否显示启动页
      show=true
      # 启动页进度条是否有回弹效果
      ...
      # 双列显示，开启务必使用等宽字体
      doubleColumn=true
      # 下载目录，默认为MUSICFOX_ROOT/download
      downloadDir=
      ...
      mpdConfigFile=/home/${user}/.config/mpd/mpd.conf
      # tcp 或 unix
      mpdNetwork=tcp
